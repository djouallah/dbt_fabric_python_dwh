def model(dbt, session):
    ...
    # Land uncompressed CSV in a dedicated folder, separate from any legacy gzip files:
    # Fabric Warehouse OPENROWSET cannot read gzip CSV (gzip forces CSV PARSER 1.0, which
    # can't tolerate the ragged/quoted AEMO rows), so the dwh port lands plain .CSV here and
    # reads it with PARSER 2.0. A matching watermark log keeps this folder's bootstrap clean.
    ...
    def download_and_extract(url, temp_dir):
        ...
        for name in z.namelist():
            if name.upper().endswith(".CSV"):
                ...
                # Land the CSV uncompressed — Fabric OPENROWSET can't read gzip CSV.
                csv_path = os.path.join(temp_dir, safe_name)
                with open(csv_path, "wb") as f:
                    f.write(z.read(name))
                results.append((name, safe_name, csv_path))
        return results
